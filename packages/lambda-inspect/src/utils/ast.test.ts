...
describe("ast utils", () => {
  describe("isHandler", () => {
    it("should identify exported function declaration", () => {
      const sourceFile = ts.createSourceFile("test.ts", "export function handler() {}", ts.ScriptTarget.Latest, true);
    ...
    it("should not identify non-exported function declaration", () => {
      const sourceFile = ts.createSourceFile("test.ts", "function handler() {}", ts.ScriptTarget.Latest, true);
      const isH = isHandler(sourceFile.statements[0], "handler");
      expect(isH).toBe(false);
    });

    it("should identify exported arrow function", () => {
      const sourceFile = ts.createSourceFile("test.ts", "export const handler = () => {}", ts.ScriptTarget.Latest, true);
      const isH = isHandler(sourceFile.statements[0], "handler");
      expect(isH).toBe(true);
    });

    it("should match handler name", () => {
      const sourceFile = ts.createSourceFile("test.ts", "export const main = () => {}", ts.ScriptTarget.Latest, true);
  ...
  describe("checkHandlerBody", () => {
    it("should detect heavy lib instantiations in handler body (arrow function)", () => {
      const sourceFile = ts.createSourceFile("test.ts", "export const handler = () => { const s3 = new S3(); }", ts.ScriptTarget.Latest, true);
    ...
    it("should detect heavy lib instantiations in handler body (function declaration)", () => {
      const sourceFile = ts.createSourceFile("test.ts", "export function handler() { const dynamodb = new DynamoDB(); }", ts.ScriptTarget.Latest, true);
    ...
    it("should ignore heavy lib instantiations outside handler body", () => {
      const sourceFile = ts.createSourceFile("test.ts", "const s3 = new S3(); export const handler = () => {}", ts.ScriptTarget.Latest, true);
  ...
  describe("checkImportForV2", () => {
    it("should detect import of aws-sdk", () => {
      const sourceFile = ts.createSourceFile("test.ts", "import AWS from 'aws-sdk';", ts.ScriptTarget.Latest, true);
    ...
    it("should detect require of aws-sdk", () => {
      const sourceFile = ts.createSourceFile("test.ts", "const AWS = require('aws-sdk');", ts.ScriptTarget.Latest, true);
    ...
    it("should not detect AWS SDK v3 imports", () => {
      const sourceFile = ts.createSourceFile("test.ts", "import { S3Client } from '@aws-sdk/client-s3';", ts.ScriptTarget.Latest, true);
      const badPractices = checkImportForV2(sourceFile.statements[0]);
      expect(badPractices).toHaveLength(0);
    });
  });
