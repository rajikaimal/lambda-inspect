...
export function checkHandlerBody(node: ts.Node, handlerName: string): string[] {
...
export function checkImportForV2(node: ts.Node): string[] {
  ...
  if (ts.isImportDeclaration(node)) {
    if (ts.isStringLiteral(node.moduleSpecifier) && node.moduleSpecifier.text === "aws-sdk") {
      badPractices.push("Importing 'aws-sdk' (AWS SDK v2) is a bad practice. Use AWS SDK v3 (e.g., '@aws-sdk/client-s3') to reduce bundle size.");
    }
  }
  ...
  if (ts.isCallExpression(node)) {
    if (ts.isIdentifier(node.expression) && node.expression.text === "require") {
      ...
      if (args.length > 0 && ts.isStringLiteral(args[0]) && args[0].text === "aws-sdk") {
        badPractices.push("Requiring 'aws-sdk' (AWS SDK v2) is a bad practice. Use AWS SDK v3 (e.g., '@aws-sdk/client-s3') to reduce bundle size.");
      }
    }
  }
